...
const statuts = ["Tous", "Ouvert", "Résolu", "Pas résolu"];
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {ticketsFiltres.map((order, idx) => (
                <TableRow key={order.id ?? order.number ?? idx}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <Badge
                      ...
                      color={
                        ...
                        ? "warning"
                        : order.gravite === "faible"
                        ? "success"
                        : order.gravite === "critique"
                        ? "errorcritique"  // ou une autre couleur que tu veux
                        : "primary"
                      }
                    >
                      {order.gravite}
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
  {order.file ? (
    order.file.match(/\.(jpg|jpeg|png|gif|webp|jfif)$/i) ? (
      <Image
        src={`http://localhost:8080${order.file}`} // important : ajoute l’URL complète vers ton backend
        alt="file"
        width={40}
        height={40}
      ...
      <a
        href={`http://localhost:8080${order.file}`}
        target="_blank"
