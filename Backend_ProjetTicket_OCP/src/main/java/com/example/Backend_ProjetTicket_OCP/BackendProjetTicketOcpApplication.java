	...
	CommandLineRunner initDatabase(InstallationRepository installationRepository) {
		return args -> {
			if (installationRepository.count() == 0) {
				...
				installationRepository.save(new Installation("Centrale �lectrique", "Zone Sud"));
				installationRepository.save(new Installation("Atelier m�canique", "Zone Est"));
				installationRepository.save(new Installation("Poste de s�curit�", "Zone Ouest"));
